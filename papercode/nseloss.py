...
class LOGNSELoss(torch.nn.Module):
    """Calculate (batch-wise) LOG-NSE Loss.

    Each sample i is weighted by 1 / (std_i + eps)^2, where std_i is the standard deviation of the 
    log-transformed discharge from the basin, to which the sample belongs.
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor, q_stds_log: torch.Tensor):
        """Calculate the batch-wise NSE Loss function.
        ...
        y_true : torch.Tensor
            Tensor containing the true discharge values
        q_stds_log : torch.Tensor
            Tensor containing the log-transformed discharge std (calculate over training period) of each sample
